...
export namespace CharacterController {
  export const get = async (
    ...
    const { id } = req.params;
    const char = await CharacterService.get(id ? ({ id: Number(id) } as GenericTypes.get) : {});

    return res
      .status(StatusCode.OK)
      .send(new PresenterFactory<{ characters: Character[]; total: number }>(char, ['SUCCESS']));
  };
  export const create = async (
    ...
    return res.status(StatusCode.OK).send(new PresenterFactory<Character>(char, ['SUCCESS']));
  };
  ...
  export const destroy = async (
    ...
    return res
      .status(StatusCode.OK)
      .send(new PresenterFactory<{ message: string }>(char, ['SUCCESS']));
  };
}
